...
- **덱빌딩** — 웨이브를 비우면 3택 보상. 새 볼 카드(기본 · 중량 · 관통 · 폭탄)로 덱을 키우거나 패시브 유물을 얻는다.
...
3. 공이 바닥을 완전히 벗어나면 턴 종료 — 쓴 카드는 버린 더미로 가고, **남은 벽돌 전부가 한 줄 내려오며 맨 윗줄에 새 행이 들어온다.** 드로우 더미가 비면 버린 더미를 섞어 다시 쓴다.
...
`split`(분열 구체)은 타입만 정의돼 있고 **미구현**이라 보상 풀에서 빠져 있다.
...
- `split`(분열) 구체 미구현 — 멀티볼은 턴 종료 판정을 바꿔야 한다.
- 1웨이브(40칸)가 2웨이브 이후(18~32칸)보다 무겁다. 패턴 순서나 초기 HP 를 조정할 여지가 있다.
- 효과음은 합성음이며 실제 청취 튜닝을 거치지 않았다. 프레임레이트도 직접 계측하지 않았다.
- 페이지를 그냥 닫아 버린 진행 중인 판의 파괴 수는 누적 기록에 반영되지 않는다.
