...
한 판은 1웨이브에서 10웨이브까지의 한 번의 도전이다 — 차분히 하면 15~20분. 판 도중 저장은 없다: 탭을 닫으면 그 판은 사라지고 기록만 남는다.
...
**7. 10웨이브 — 최종 보스.** 코어 48 HP, 모든 벽돌 +6 HP, 세 줄 낮은 시작 — 여유 다섯 턴. 깨면 **승리**.
...
5. 필드의 벽돌을 전부 파괴하면 웨이브 클리어 → 보상 3택(볼 또는 유물, 스킵 가능) → 다음 웨이브. 5·10웨이브는 **보스 웨이브**. 10웨이브를 클리어하면 승리.
...
- 보스 웨이브와 두 번째 볼·유물 묶음은 봇으로만 맞췄다("보스 웨이브" 절). 공을 잃을 때마다 2씩 회복하는 코어가 사람에게 공정하게 느껴지는지는 모른다.
